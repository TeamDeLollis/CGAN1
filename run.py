...
def load_dataset(data_dir, data_type, img_width, img_height):
    ...
    facade_photos_h5 = [f for f in os.listdir(data_type + 'A') if '.jpg' in f]
    facade_labels_h5 = [f for f in os.listdir(data_type + 'B') if '.jpg' in f]

    final_facade_photos = None
    final_facade_labels = None

    for index in range(len(facade_photos_h5)):
        facade_photos_path = data_dir + '/' + data_type + 'A' + '/' + facade_photos_h5[index]
        facade_labels_path = data_dir + '/' + data_type + 'B' + '/' + facade_labels_h5[index]
        ...
        for image_name in facade_photos_h5:
            image = Image.open(image_name)
            facade_photos.append(image)

        all_facades_photos = np.array(facade_photos)

        for image_name in facade_labels_h5:
            image =Image.open(image_name)
            image2_vec = np.array(image)
            facade_labels.append(facade_labels)

        all_facades_labels = np.array(fa)


        # Resize and normalize images
        num_photos = len(facade_photos_h5)
        num_labels = len(facade_labels_h5)
...
if __name__ == '__main__':
    ...
    """
    training_facade_photos, training_facade_labels = load_dataset(data_dir=dataset_dir, data_type='Train',
                                                                  img_width=img_width, img_height=img_height)

    test_facade_photos, test_facade_labels = load_dataset(data_dir=dataset_dir, data_type='Test',
                                                          img_width=img_width, img_height=img_height)

    validation_facade_photos, validation_facade_labels = load_dataset(data_dir=dataset_dir, data_type='Test',
    img_width=img_width, img_height=img_height)
